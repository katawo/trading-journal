...
- **Restart the ingestion API after replacing a user's database file:** it caches one initialized `SQLiteJournalRepository` per user for the life of the process (`ingestion_api.py::_user_repository`), keyed by the resolved database path. If you restore a backup, swap in a different `.db` file, or otherwise replace `trading_journal.db` for a user out from under a running process, that user's cached connections keep writing to the original file handle and the replacement is silently ignored until the ingestion process restarts.
