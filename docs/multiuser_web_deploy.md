...
4. The EA's on-chart status comment shows whether a backend URL is configured. Closed positions push on the periodic safety timer (not on every trade event, so a slow or unreachable backend can never stall trade handling) — expect up to `InpSafetyExportSeconds` (default 60s) of delay after a trade closes.
